...
def twomass(dr='tgas'):
    ...
    #-----------------------------John modification---------------------------- 
    if dr == 'tgas':
        filePath = os.path.join(os.sep, 'Users','john','Desktop','bovy_code', \
                         'gaia_tools-master', 'Gaia', 'dstn_match''', 'tgas-matched-2mass.fits.gz')     #PUT YOUR OWN PATH
        return fitsio.read(filePath,1)
        
    if dr == 'hp_short':
        filePath = os.path.join(os.sep, 'Users','john','Desktop','bovy_code', \
                         'gaia_tools-master', '2mass', '2mass.fits' )#  PUT YOUR OWN PATH
        return fitsio.read(filePath,ext=1)
               
    if dr == 'hp':
        filePaths = []
        for i in range(0,38):
           filePaths.append(os.path.join(os.sep, 'Users','john','Desktop','bovy_code', \
                         'gaia_tools-master', '2mass', '2mass_source_j_14','twomass_part%.2i-result.fits' % i) )#  PUT YOUR OWN PATH
...
def gaia(dr=1):
    ...
    if dr == 1:
       filePath = os.path.join(os.sep, 'Users','john','Desktop','bovy_code',
                         'gaia_tools-master', 'Gaia', 'gaia_dr%i.fits' % dr)
       return fitsio.read(filePath,ext=1)
    if dr == 2:
       filePaths = []
       for i in range(46) : #20
           #filePaths.append(os.path.join(os.sep, 'Users','john','Desktop','bovy_code', 'gaia_tools-master', 'Gaia', 'gaia_dr2_tm', 'gaia_dr2_part%.2i-result.fits' % i) )
           filePaths.append(os.path.join(os.sep, 'Users','john','Desktop','bovy_code', 'gaia_tools-master', 'Gaia', 'gdr2_mag_cut', 'gaia_part%.2i-result.fits' % i) )
       return numpy.lib.recfunctions.stack_arrays(\
               [fitsio.read(filePath,ext=1) for filePath in filePaths],
               autoconvert=True)
    #--------------------------------------------------------------------------- 
