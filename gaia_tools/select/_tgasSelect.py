...
class tgasSelect(object):
    def __init__(self,
                 ...
                 max_scd=0.7,
                 min_lat=20.,
                 jmin=2.,jmax=13.5,jkmin=-0.05,jkmax=1.00):
        """#
        NAME:
           __init__
        PURPOSE:
        ...
        self._full_tgas = gaia_tools.load.tgas() #  John: dr2/tgas catalog matched with twomass
        self._full_tgas = self._full_tgas.data
        #self._full_twomass =  gaia_tools.load.gaia(dr=2) #  John: twomass catalog matched with /dr2tgas
        self._full_twomass  =  gaia_tools.load.twomass('hp')
        self._full_twomass  =  self._full_twomass.data
        indx_j_range = (self._full_twomass['j_mag'] < 13.5)*(self._full_twomass['j_mag'] > 2.)
        self._full_twomass = self._full_twomass[indx_j_range]
        
        m1,m2,sep= gaia_tools.xmatch.xmatch(self._full_twomass,self._full_tgas,
                                            colRA1='ra',colDec1='dec',
                                            colRA2='ra',colDec2='dec',
                                            epoch1=2000.,epoch2=2015.0,swap=True) #2015.5 for DR2
        self._full_twomass=self._full_twomass[m1]
        self._full_tgas=self._full_tgas[m2]
        self._complete_twomass  =  gaia_tools.load.twomass('hp') #  John: the whole twomass catalog
        self._complete_twomass  =  self._complete_twomass.data
        ...
        #self.create_healpix_map(self._ext_jt, self._ext_jk, self._complete_twomass['ra'], self._complete_twomass['dec'],
        #    jmin=jmin,jmax=jmax,jkmin=jkmin,jkmax=jkmax)
        
        #  John: Some overall statistics to aid in determining the good sky, setup 
    ...
    def _determine_selection(self,jmin,jmax,jkmin,jkmax):
        ...
        if to_plot:
            jk_plot =numpy.linspace(self._jkmin,self._jkmax,_njkbin-1)
            j_plot = numpy.linspace(2,14,100)
            jkbin_plot= numpy.floor((_njkbin-1)*(jk_plot-self._jkmin)\
                                   /(self._jkmax-self._jkmin)).astype('int')
            plt.figure(figsize=(5,5))
            plt.title('Completeness Volume'); plt.xlabel('$J-K_s$'); plt.ylabel('$J$')
            fit_completeness = []
            for ii , jk_bin in enumerate(jk_plot):
                #if ii == 0 or ii == len(jkbin_plot)-1: continue
                fit_completeness_row = [float(sf_splines[jkbin_plot[ii]]( jt(jk_bin, j_plot[jj])) ) for jj in range(len(j_plot))]
            ...
            plt.imshow(numpy.transpose(fit_completeness), interpolation='nearest', origin='low', cmap='viridis',
                     extent=[jk_plot[0]+djk, jk_plot[-1]-djk, j_plot[0], j_plot[-1]], aspect='auto')
            cbar = plt.colorbar(); cbar.set_label("completeness after spline fit")
            plt.show()
...
"""    def plot_2mass(self,jmin=None,jmax=None,
    ...
    def plot_magdist(self,type='sf',cut=True,splitcolors=False,overplot=False):
        ...
        jtbins= (numpy.amax(_2mc[0])-numpy.amin(_2mc[0]))/0.1+1
        nstar2mass, edges= numpy.histogramdd(\
            _2mc[:3].T,bins=[jtbins, _njkbin,_BASE_NPIX],
            range=[[numpy.amin(_2mc[0])-0.05,numpy.amax(_2mc[0])+0.05],
                   [-0.05,1.0],[-0.5,_BASE_NPIX-0.5]],weights=_2mc[3])
        findx= (self._full_jk > -0.05)*(self._full_jk < 1.0)\
            *(self._full_twomass['j_mag'] < 13.5)
